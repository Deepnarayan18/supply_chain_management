...
USER_AGENT = "supply_chain_monitor_v11/deepghosh@youremail.com"

# API KEYS
OPENWEATHER_API_KEY = "789f1ee0f9eec1e1b1dfbb9ab1076273"
NEWS_API_AI_KEY = "a5e5016f-b8e4-4d6a-905c-d5181d37e9f8" 
...
async def fetch_real_news_for_location(full_address: str) -> str:
    ...
    payload = {
        "action": "getArticles",
        "keyword": f"{city_name} traffic OR logistics OR transport OR strike OR delay",
        "articlesPage": 1,
        "articlesCount": 1,
        "articlesSortBy": "date",
                    ...
                    if isinstance(data, dict):
                        ...
                        if isinstance(articles_obj, dict):
                            ...
                            if isinstance(results, list) and len(results) > 0:
                                ...
                                source = first_article.get("source", {}).get("title", "Unknown Source")
...
def get_llm_risk_assessment(location: str, lat: float, lon: float, mode: str, temp: Any, condition: str, waves: str, news: str) -> Dict[str, str]:
    ...
    try:
        ...
        # Safely handle None returns from Groq
...
def main():
    ...
    st.markdown("Track **Roadways** or **Seaways**. Powered by Groq (Llama-3) & NewsAPI.ai for Risk Intelligence.")
    ...
    if analyze_btn:
        with st.spinner(f"Analyzing {mode} via Groq Llama-3 AI..."):
            ...
            # --- PLOTLY INTERACTIVE MAP (RE-ADDED!) ---
