...
if dotenv_path:
    ...
    st.warning(".env file not found automatically. Make sure it is named exactly .env and is in the same folder as app4.py")

TIMEOUT = 15
MAX_RETRIES = 3
OSRM_BASE_URL = "https://router.project-osrm.org"
USER_AGENT = "supplychainmonitorv4.6_deepghosh@youremail.com"

OPENWEATHER_API_KEY = os.getenv("OPENWEATHER_API_KEY", "")
GROQ_API_KEY = os.getenv("GROQ_API_KEY", "")

if not GROQ_API_KEY or not OPENWEATHER_API_KEY:
    st.warning("API keys are missing! Please ensure OPENWEATHER_API_KEY and GROQ_API_KEY are set in your .env file.")
...
async def fetch_real_news_for_location(full_address: str) -> str:
    if not full_address:
        return "No disruptive news detected recently."
...
def get_llm_risk_assessment(location: str, lat: float, lon: float, mode: str, temp: Any, condition: str, waves: str, news: str) -> Dict[str, str]:
    if not groq_client:
        return {"Risk": "Medium", "Details": "API Key missing. Cannot generate AI risk assessment."}
...
def generate_rerouting_suggestion(milestones: List[Dict[str, Any]], mode: str, origin: str, dest: str, primary_distance: float, alt_info: str, alt_milestones: List[Dict[str, Any]]) -> str:
    if not groq_client:
        return "API Key missing. Cannot generate intelligence briefing."
    if not milestones:
    ...
    context = "\n".join([f"- High Risk at {m.get('Location', 'Unknown')}: {m.get('AI Intelligence', '')} (Weather: {m.get('Temp C', '')}°C, {m.get('Weather', '')})" for m in milestones if m.get("Risk Level") in ["High", "Critical"]])
    if not context:
    ...
    alt_risk_pct = 0.0
    has_alt_route = len(alt_milestones) > 0
    ...
    if has_alt_route:
        ...
        alt_context = "\n".join([f"- Alt Region: {am.get('Location', 'Unknown')}: {am.get('Risk Level', 'Low')} Risk. (Weather: {am.get('Weather', '')}, Temp: {am.get('Temp C', '')}°C) Intel: {am.get('Local News', '')}" for am in alt_milestones if am.get("Risk Level") in ["High", "Critical", "Medium"]])

    if has_alt_route:
        prompt = f"""
System Persona: You are a Chief Logistics Data-Geography Analyst. You operate on cold mathematical logic.

Task: Execute a comprehensive risk audit and deliver a definitive routing mandate in JSON format.
Data Feeds:
...
Evaluate the routes and provide a JSON response with this exact structure:
{{
    "Briefing_Assessment": "Deliver a highly detailed clinical analysis of the primary path based on the telemetry provided. Note the exact risk percentage calculated ({primary_risk_pct:.1f}%). Explain specific weather, geographical or news-based threats.",
    "Alternative_Assessment": "Deliver a highly detailed clinical analysis of the alternative path. Note the exact risk percentage calculated ({alt_risk_pct:.1f}%). Explain why it is safer or riskier.",
    "Definitive_Mandate": "State exactly which route to take in one clear, authoritative sentence based strictly on the lower risk percentage.",
    "Mathematical_Justification": "Explain your mandate in detail. Explicitly compare the two risk percentages and discuss the operational impact of the difference.",
    "Chart_Insight_Summary": "Write a detailed 3-5 sentence technical summary comparing the primary and alternative routes, justifying the chosen route based on risk and/or distance.",
    "Risk_Matrix_Insight": "Write a 2-3 sentence analysis comparing the absolute volume of severe vs moderate bottlenecks on both vectors to explain the risk distribution."
}}
"""
    else:
        prompt = f"""
...
Evaluate the primary route and provide a JSON response with this exact structure:
{{
    "Briefing_Assessment": "Deliver a highly detailed clinical analysis of the primary path based on the telemetry provided. Note the exact risk percentage calculated ({primary_risk_pct:.1f}%). Explain specific weather, geographical or news-based threats.",
    "Alternative_Assessment": "State clearly that no viable alternative route is available for this corridor.",
    "Definitive_Mandate": "State exactly that the primary route must be taken as there are no alternatives.",
    "Mathematical_Justification": "Explain the mandate. State the primary route's risk factor ({primary_risk_pct:.1f}%) and note that without an alternative, this risk must be managed operationally. DO NOT compare it to an alternative route.",
    "Chart_Insight_Summary": "Write a detailed 3-5 sentence technical summary of the primary route's conditions and risk profile. Emphasize operational metrics like total distance and estimated transit time.",
    "Risk_Matrix_Insight": "Write a 2-3 sentence analysis breaking down the severe vs moderate bottlenecks on the primary route. Highlight the specific count of critical nodes."
}}
"""
    ...
    try:
        ...
        if content:
            try:
                ...
                st.session_state.chart_insight = json_data.get("Chart_Insight_Summary", "")
                st.session_state.risk_matrix_insight = json_data.get("Risk_Matrix_Insight", "")
                
                formatted_response = f"""► Primary Route Assessment
{json_data.get('Briefing_Assessment', '')}

► Alternative Route Assessment
{json_data.get('Alternative_Assessment', '')}

► Definitive Mandate
{json_data.get('Definitive_Mandate', '')}

► Mathematical Justification
{json_data.get('Mathematical_Justification', '')}
"""
                return formatted_response.strip()
            except json.JSONDecodeError:
                return "Strategic intelligence briefing unavailable (Invalid JSON from API)."
        else:
            return "Strategic intelligence briefing unavailable."
    except Exception as e:
         return f"Strategic intelligence briefing unavailable at this time. (API Error: {e})"

async def geocode_place_arcgis(placename: str, geolocator: ArcGIS) -> Optional[Dict[str, Any]]:
    if not placename: return None
    for attempt in range(MAX_RETRIES):
        try:
            location = await geolocator.geocode(placename, exactly_one=True, timeout=TIMEOUT) # type: ignore
...
async def calculate_road_route(src: str, dest: str) -> Dict[str, Any]:
    async with ArcGIS(adapter_factory=AioHTTPAdapter) as geolocator:
            ...
            if alt_coords:
                ...
                if len(alt_coords) >= 10:
                    ...
                    for pt in sample_pts:
                        ...
                        try:
                            rev = await geolocator.reverse((lat, lon), timeout=TIMEOUT) # type: ignore
                        ...
                        if place not in alt_places:
                            ...
                            alt_milestones.append({
                                ...
                                "Lat": lat,
                                "Lon": lon,
                                "Temp C": weather.get("temp", "N/A"),
                                "Weather": weather.get("condition", "N/A"),
                                "Local News": local_news,
                                "Risk Level": llm_eval.get("Risk", "Medium"),
                                "AI Intelligence": llm_eval.get("Details", "")
                            })
        ...
        return {
            "coords": coords, "alt_coords": alt_coords,
            "distance": primary_distance, "primary_duration": primary_duration,
            "alt_distance": alt_distance, "alt_duration": alt_duration,
            "milestones": milestones, "alt_milestones": alt_milestones,
            "alt_info": alt_info
        }
...
async def calculate_sea_route(src: str, dest: str) -> Dict[str, Any]:
    async with ArcGIS(adapter_factory=AioHTTPAdapter) as geolocator:
        ...
        return {
            "coords": coords, "alt_coords": [],
            "distance": len(coords), "primary_duration": 0,
            "alt_distance": 0, "alt_duration": 0,
            "milestones": milestones, "alt_milestones": [], "alt_info": ""
        }
...
def generate_matplotlib_charts(df: pd.DataFrame, result: Dict[str, Any], mode: str):
    ...
    with col2:
        ...
        risk_counts = {r: risk_list.count(r) for r in set(risk_list)}
    ...
    has_alt_route = alt_dist > 0 and mode == "Roadways" and len(alt_milestones) > 0
    ...
    with col5:
         ...
         chart_insight_text = st.session_state.get('chart_insight', 'Chart insight data not available.')
         ...
         if has_alt_route:
             ...
             for i, val in enumerate(alt_vals):
                 if val > 0:
                     ax5.text(i + width_bar/2, val + max_height * 0.02, f"Alt: {val:.1f}", ha='center', fontweight='bold')
             for i, val in enumerate(primary_vals):
                 if val > 0:
                     ax5.text(i - width_bar/2, val + max_height * 0.02, f"Prim: {val:.1f}", ha='center', fontweight='bold')
         else:
             ax5.bar(x, primary_vals, width_bar, label='Primary', color='#0066cc')
             max_height = max(primary_vals) if max(primary_vals) > 0 else 1
             for i, val in enumerate(primary_vals):
                 if val > 0:
                     ax5.text(i, val + max_height * 0.02, f"Prim: {val:.1f}", ha='center', fontweight='bold', color='#1E293B')
    ...
    with col6:
         ...
         risk_matrix_text = st.session_state.get('risk_matrix_insight', 'Comparing absolute volume of severe vs moderate bottlenecks detected live on vectors.')
         st.info(f"💡 **Risk Matrix Insight:** {risk_matrix_text}")
         ...
         if has_alt_route:
             ...
             for i, val in enumerate(prim_counts):
                 if val > 0:
                     ax6.text(i - width_bar/2, val + 0.1, f"Prim: {val}", ha='center', fontweight='bold')
             for i, val in enumerate(alt_counts):
                 if val > 0:
                     ax6.text(i + width_bar/2, val + 0.1, f"Alt: {val}", ha='center', fontweight='bold')
         else:
             ax6.bar(x_risk, prim_counts, width_bar, label='Primary', color='#0066cc')
             max_y = max(prim_counts) if prim_counts else 0
             for i, val in enumerate(prim_counts):
                 if val > 0:
                     ax6.text(i, val + 0.1, f"Prim: {val}", ha='center', fontweight='bold')
...
def main():
    st.markdown("""
        ...
        .main-header { font-family: 'Inter', sans-serif; font-size: 2.2rem; font-weight: 700; color: #0f172a; margin-bottom: 0.2rem; letter-spacing: -0.02em; }
        .sub-header { font-family: 'Inter', sans-serif; color: #475569; font-size: 1.1rem; margin-bottom: 2rem; }
        .strategy-box { background-color: #ffffff; border-left: 4px solid #0284c7; border-top: 1px solid #e2e8f0; border-right: 1px solid #e2e8f0; border-bottom: 1px solid #e2e8f0; padding: 1.5rem; border-radius: 6px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); font-family: 'Inter', sans-serif; font-size: 1rem; line-height: 1.6; color: #334155; }
        [data-testid="stSidebar"] { background-color: #ffffff; border-right: 1px solid #e2e8f0; }
        .stButton>button { background: #0284c7; color: white; border: none; border-radius: 4px; padding: 0.5rem 1rem; font-weight: 600; font-family: 'Inter', sans-serif; width: 100%; transition: all 0.3s ease; }
        .stButton>button:hover { background: #0369a1; box-shadow: 0 4px 12px rgba(2, 132, 199, 0.2); color: white; }
        </style>
    """, unsafe_allow_html=True)
    ...
    if 'chart_insight' not in st.session_state:
        st.session_state.chart_insight = ""
    if 'risk_matrix_insight' not in st.session_state:
        st.session_state.risk_matrix_insight = ""
    ...
    if st.session_state.report_generated:
        ...
        st.markdown(f'<div class="strategy-box">{st.session_state.reroute_strategy.replace("\\n", "<br>").replace("►", "<b><span style=\'color:#0284c7\'>►</span></b>")}</div>', unsafe_allow_html=True)
        ...
        colormap = {'Low': 'green', 'Medium': 'orange', 'High': 'red', 'Critical': 'darkred'}
        ...
        if folium_coords:
            route_line = folium.PolyLine(locations=folium_coords, color="#0ea5e9", weight=4, opacity=0.8, tooltip=f"Primary {mode} Vector")
            route_line.add_to(m)
            plugins.PolyLineTextPath(route_line, "        ►        ", repeat=True, offset=6, attributes={'fill': '#0ea5e9', 'font-weight': 'bold', 'font-size': '14'}).add_to(m)
        ...
        if result.get("alt_coords"):
            ...
            if alt_folium_coords:
                 folium.PolyLine(locations=alt_folium_coords, color="#f59e0b", weight=3, opacity=0.7, dash_array="5, 5", tooltip="Alternative Vector").add_to(m)
        ...
        def generate_popup_html(location, risk, temp, weather, news, intel, border_color):
            return f"""
                <div style="width:260px; background-color:#ffffff; color:#334155; padding:10px; border-radius:5px; border-left:4px solid {border_color}; font-family:sans-serif; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                    ...
                    <span style="font-size:0.9rem;"><b>Risk:</b> <span style="color:{border_color}; font-weight:bold;">{risk}</span></span><br/>
        ...
        for row in result.get("alt_milestones", []):
             ...
             marker_color = colormap.get(risk_level, 'gray')
        ...
        with tab1:
            ...
            if "Temp C" in df_primary.columns:
                 df_primary["Temp C"] = df_primary["Temp C"].astype(str)
            if "Wave Height" in df_primary.columns:
        ...
        if "Temp C" in df_for_charts.columns:
             df_for_charts["Temp C"] = df_for_charts["Temp C"].astype(str)
        if "Wave Height" in df_for_charts.columns:
